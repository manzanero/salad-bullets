...
@onready var left_gun := %LeftHand/Gun as Gun
@onready var right_gun := %RightHand/Gun as Gun


func _ready() -> void:
	_webxr_ready()
	
	#%LeftController.button_pressed.connect(_on_left_controller_button_pressed)
	#%RightController.button_pressed.connect(_on_right_controller_button_pressed)
	webxr_interface.selectstart.connect(_on_select_start)
...
# from here is for WebXR

var webxr_interface
 

func _webxr_ready() -> void:
